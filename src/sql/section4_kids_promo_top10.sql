...
WITH promo_map AS (
  ...
  UNION ALL SELECT '1450', '경량패딩 테스트', '/display/promotions/event/1450'
  ...
  UNION ALL SELECT '1516', '이현이도', '/display/promotions/event/1516'
  ...
  UNION ALL SELECT '1579', '기프트 이벤트', '/display/promotions/event/1579'
  ...
  UNION ALL SELECT '1601', '소히조이 사전알림', '/display/promotions/event/1601'
  UNION ALL SELECT '1600', '바시티 에프터 스쿨', '/display/promotions/collection/1600'
),
...
events_with_location AS (
  SELECT
    ...
    e.EVENT_TIMESTAMP,
    e.EVENT_NAME,
    MAX(IFF(ep_p.value:key::STRING='ga_session_id', ep_p.value:value:int_value::NUMBER, NULL)) AS session_id,
    MAX(IFF(ep_p.value:key::STRING='page_location', ep_p.value:value:string_value::STRING, NULL)) AS page_location
  FROM FNF.STRG_GA.EVENTS e,
       LATERAL FLATTEN(input => e.EVENT_PARAMS) AS ep_p  /* ✅ ep -> ep_p 로 변경 */
  WHERE e.P_BRAND='M'
...
purchase_event_base AS (
  SELECT
    e.USER_PSEUDO_ID,
    e.EVENT_TIMESTAMP,
    MAX(IFF(ep_p2.value:key::STRING='ga_session_id', ep_p2.value:value:int_value::NUMBER, NULL)) AS session_id,
    SUM(COALESCE(it_i.value:item_revenue::NUMBER, COALESCE(it_i.value:price::NUMBER,0) * COALESCE(it_i.value:quantity::NUMBER,1), 0)) AS event_revenue
  FROM FNF.STRG_GA.EVENTS e,
       LATERAL FLATTEN(input => e.EVENT_PARAMS) AS ep_p2, /* ✅ ep -> ep_p2 로 변경 */
       LATERAL FLATTEN(input => e.ITEMS) AS it_i
  WHERE e.P_BRAND='M'
    AND e.P_DATE BETWEEN %(start_date)s AND %(end_date)s
    AND e.EVENT_NAME='purchase'
    AND LEFT(it_i.value:item_id::STRING, 1) = '7'
...
promo_purchase_events AS (
  SELECT
    ps.promo_key,
    COUNT(DISTINCT (ps.USER_PSEUDO_ID || '-' || ps.session_id || '-' || peb.EVENT_TIMESTAMP)) AS purchase_events,
...
SELECT
  ...
  CASE
    ...
    ELSE ROUND((COALESCE(ppe.purchase_events, 0)::FLOAT / NULLIF(vie.view_item_events, 0)::FLOAT) * 100, 2)
  END AS purchase_cvr_pct,
  COALESCE(ppe.revenue, 0) AS revenue
FROM session_metrics sm
JOIN promo_map pm ON sm.promo_key = pm.promo_key
LEFT JOIN view_item_events vie ON sm.promo_key = vie.promo_key
LEFT JOIN promo_purchase_events ppe ON sm.promo_key = ppe.promo_key
LEFT JOIN purchase_sessions pss ON sm.promo_key = pss.promo_key
