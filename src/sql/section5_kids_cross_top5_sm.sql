/* section5_kids_cross_top5_sm.sql
   성인광고(M_) 유입이 만든 키즈(7*) 매출 기준 source/medium TOP5 + inbound sessions
*/
...
inbound AS (
  ...
  WHERE LEFT(UPPER(TRIM(campaign)),2)='M_'
  GROUP BY 1,2
),
...
rev AS (
  ...
  WHERE LEFT(UPPER(TRIM(d.campaign)),2)='M_'
  GROUP BY 1,2
)
