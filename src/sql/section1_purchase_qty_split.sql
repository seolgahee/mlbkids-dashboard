WITH purchase_items AS (
  ...
  SELECT
    ...
    SUM(COALESCE(it.value:quantity::NUMBER, 1)) AS purchase_qty
...
pv AS (
  -- 3) 세션 이벤트에서 캠페인 가져오기 (purchase에는 manual_campaign_name이 없어서 여기서 가져와야 함)
  SELECT
    e.USER_PSEUDO_ID,
    e.EVENT_TIMESTAMP,
    e.EVENT_PARAMS,
...
-- 5) 키즈 상품 구매 세션에 캠페인을 붙여서 I_ vs 그 외로 분리
SELECT
  CASE
